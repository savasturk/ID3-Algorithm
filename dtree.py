...
def create_decision_tree(examples, attributes, target_attribute, heuristic_funtion):
	...
	# make copies of data since python list are variable
...
def get_examples(data, attr, value):
    ...
    if not data:
        ...
        if record[attr] == value:
            ...
            # extend function meaning appending elements from the iterable.
            return rtn_lst
        else:
            rtn_lst.extend(get_examples(data, attr, value))
            return rtn_lst
...
def get_classification(record, tree):
    ...
    """
    # If the current node is a string, then we've
    # ! reached a leaf node and
    ...
    # Traverse the tree further until a leaf node is found.
        ...
        attr = tree.keys()[0]
        tempTree = tree[attr][record[attr]]
        return get_classification(record, tempTree)
